...
// Copy constructor
{
    // Cria uma nova lista encadeada como cópia profunda da outra
...
// Perform a deep copy of the object
{
    ...
    // Copy primitive data members (simple values)
    id = other.id;
    ...
    // Free the previously allocated memory (to avoid memory leaks)
    if (route != nullptr)
        delete route;

    // Deep copy of the linked list. Ensures the current object gets its own separate copy of the list
    route = new LinkedList(*other.route);
...
// returns the next step in route
...
*/
{ 
    current_location_id = route->Search(0);
    route->Remove(0);
